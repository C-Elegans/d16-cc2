        ...
        for(int i = 0; i < instructions.size() - 1; i++) {
            if(instructions[i]->type == PUSH && instructions[i + 1]->type == POP) {
                if(instanceOf<Instruction_RI>(&(*instructions[i]))) {
                    ...
                    std::unique_ptr<Instruction_RI> inst =
                        std::make_unique<Instruction_RI>(MOV,
    ...
    passes.push_back([](instruction_vec instructions) {
        for(int i = 0; i < instructions.size() - 1; i++) {
            if(instructions[i]->type == MOV) {
                if(instanceOf<Instruction_RI>(&(*instructions[i])) &&
                   instanceOf<Instruction_RR>(&(*instructions[i + 1]))) {
                    ...
                    if(rd_1 == rs_2) {
                        ...
                        instructions.insert(instructions.begin() + i,
                                            std::make_unique<Instruction_RI>(type, rd_2, imm));
                    }
                }
            }
        }
